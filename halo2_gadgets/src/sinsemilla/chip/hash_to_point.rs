...
pub(crate) fn verify_sinsemilla_hash(
    message: &Message<Base, K, { sinsemilla::C }>,
    Q: EccPointQ,
    x_a: X<Base>,
...
pub(crate) fn hash_all_pieces<C, Hash, Commit, F>(
    sinsemilla_chip: &C,
    region: &mut Region<'_, pallas::Base>,
    mut offset: usize,
    message: &Message<Base, K, { sinsemilla::C }>,
...
>
where
    C: SinsemillaChipProps<Hash, Commit, F>,
    Hash: HashDomains<pallas::Affine>,
    F: FixedPoints<pallas::Affine>,
    Commit: CommitDomains<pallas::Affine, F, Hash>,
{
    let config = sinsemilla_chip.config();
    ...
    let y_a = {
        ...
        // multiplied by zero.)
        {
            region.assign_advice(
                || "dummy lambda2",
                config.base().double_and_add.lambda_2,
                offset,
...
pub(crate) fn hash_piece<C, Hash, Commit, F>(
    sinsemilla_chip: &C,
    region: &mut Region<'_, pallas::Base>,
    offset: usize,
    piece: &MessagePiece<Base, K>,
    mut x_a: X<pallas::Base>,
    mut y_a: Y<pallas::Base>,
    final_piece: bool,
) -> Result<
    ...
    Commit: CommitDomains<pallas::Affine, F, Hash>,
{
    ...
    // Selector assignments
    {
        ...
        for row in 0..piece.num_words() {
            config.base().q_sinsemilla1.enable(region, offset + row)?;
        }
    ...
    for (row, gen) in generators.iter().enumerate() {
        ...
        let lambda_2 = {
            let lambda_2 = y_a.0 * pallas::Base::from(2) * (x_a.value() - x_r).invert() - lambda_1;
        ...
        let y_a_new: Y<pallas::Base> = (lambda_2 * (x_a.value() - x_a_new.value()) - y_a.0).into();
